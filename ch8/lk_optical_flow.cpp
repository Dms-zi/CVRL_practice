...
// Bi-linear interpolation is used to get the pixel value
inline float GetPixelValue(const cv::Mat &img, float x, float y) {
    // boundary check
    // clamping the value to [0, img.cols-1] and [0, img.rows-1]
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (x >= img.cols - 1) x = img.cols - 2;
    if (y >= img.rows - 1) y = img.rows - 2;
    
    float xx = x - floor(x); // floor(x) is the left pixel
    float yy = y - floor(y);
...
int main(int argc, char **argv) {
    ...
    cv::imwrite("tracked_opencv.png", img2_CV);
    cv::waitKey(0);
